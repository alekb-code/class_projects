...
arguments
    ...
    mu (1,1) {mustBeGreaterThanOrEqual(mu,1),mustBeLessThanHalf(mu,N)}
end
...
function mustBeLessThanHalf(mu,N)
    if mu > N/2
        eidType = 'mustBeLessThanHalf:notLessThanHalf';
        msgType = 'Expected degree must be <= %.1f.';
        error(eidType,msgType,N/2);
    end
end
